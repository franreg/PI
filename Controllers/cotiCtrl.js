...
// Concerne les notes de controles continus 
module.exports= {
    ...
    register: async(req,res)=>{
        ...
        try {
            ...
            const notecotienFound = await models.notecoti.findOne({ where: { MaquetteId: MaquetteId, EtudiantId:EtudiantId, ECUEId:ECUEId} });
            if (notecotienFound) {
                return res.status(409).json({ error: "Cet etudiant a déjà une note" });
            }
            // Ajouter l'élément dans la base de données
            const newnotecotien = await models.notecoti.create({ MaquetteId,EtudiantId, ECUEId,note });
            return res.status(201).json({ notecotienId: newnotecotien.id, message: "Note ajoutée avec succès" });
        } catch (error) {
    ...
    update: async(req,res)=>{
        ...
        try {
            // Vérifier que le note n'existe pas déja
            const notecotienFound = await models.notecoti.findOne({ where: {id:id} });
            if (!notecotienFound) {
                return res.status(409).json({ error: "Cette note n' existe déjà" });
            }
            // Modifier l'élément dans la base de données
            await models.notecoti.update({ MaquetteId,EtudiantId, ECUEId,note },{ where: {id:id} });
    ...
    delete : async(req,res)=>{
        ...
        try {
            ...
            // Supprimer l'élément dans la base de données
             await models.notecoti.destroy({ where: {id:id} });
    ...
    getOne: async(req,res)=>{
        ...
        try {
            ...
            return res.status(201).json({  message: notecotienFound });
        } catch (error) {
    ...
    getbyIDEtudiant: async(req,res)=>{
        ...
        try {
            // Vérifier que le note n'existe pas déja
            const notecotienFound = await models.notecoti.findAll({ where: {EtudiantId:id} });
            if (notecotienFound.lengh<0) {
    ...
    AllNoteCoti: async(req,res)=>{
        try {
            const exam = await models.notecoti.findAll();
